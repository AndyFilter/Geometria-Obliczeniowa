...
int OnGui()
{
    ...
    if(ImGui::BeginTabBar("Items")) {
        ...
        if(ImGui::BeginTabItem("Twórz siatkę")) {
            ...
            if(ImGui::BeginChild("Parameters", {500, -1}, ImGuiChildFlags_Border)) {
                ...
                if(ImGui::Button("Wyeksportuj", {-1, 0})) {
                    constructed_mesh.Export(OUT_FILE_POINTS, OUT_FILE_ELEMS, false);
                }
            ...
            if(ImGui::BeginChild("Canvas", {-1, -1}, ImGuiChildFlags_Border)) {
                ...
                //if(ImGui::DrawPoint(ImGui::Local2Canvas(p1, avail, startPos), "p1", dl, POINT_BASE_RADIUS, constructed_mesh._IsInsideMesh(p1) ? POINT_SPECIAL_COLOR : POINT_BASE_COLOR) && ImGui::IsMouseClicked(0)) {
                //    dragging_point = &p1;
                //}
        ...
        if(ImGui::BeginTabItem("Otwórz siatkę")) {
            static TriangleMesh mesh = TriangleMesh("../Export/mesh_points.dat", "../Export/mesh_elements.dat", {1,1});
            ...
            if(ImGui::BeginChild("Parameters", {500, -1}, ImGuiChildFlags_Border)) {
                ...
                if(ImGui::Button("Wczytaj", {-1, 0})) {
                    mesh = TriangleMesh("../Export/mesh_points.dat", "../Export/mesh_elements.dat", {1,1});
                }
            ...
            if(ImGui::BeginChild("Canvas", {-1, -1}, ImGuiChildFlags_Border)) {
                ...
                for(int i = 0; i < mesh.elements.size(); i++) {
                    ...
                    Vec2 pos1 = ImGui::Local2Canvas(mesh.nodes[e.points[0]], avail, startPos);
                    Vec2 pos2 = ImGui::Local2Canvas(mesh.nodes[e.points[1]], avail, startPos);
                    Vec2 pos3 = ImGui::Local2Canvas(mesh.nodes[e.points[2]], avail, startPos);
                ...
                for(auto& edge : mesh.poly.edges) {
                    Vec2 pos1 = ImGui::Local2Canvas(mesh.poly.points[edge.start], avail, startPos);
                    Vec2 pos2 = ImGui::Local2Canvas(mesh.poly.points[edge.end], avail, startPos);
                    dl->AddLine(pos1, pos2, LINE_SPECIAL_COLOR, 1);

                    char _buf[12];
                    sprintf(_buf, "%i", edge.start);
                    if(ImGui::DrawPoint(pos1, _buf, dl) && ImGui::IsMouseClicked(0)) {
                        dragging_point = &mesh.poly.points[edge.start];
                    }
                }
